...
RUN mkdir -p /data/conf/php-fpm.d/ && mkdir -p /data/tmp/ && mkdir -p /data/logs
...
curl -sS https://getcomposer.org/installer | php7 -- --install-dir=/usr/bin --filename=composer  && rm -rf /var/cache/apk/* \ 
    ...
    && sed -i "s|;opcache.file_cache=.*|opcache.file_cache=/data/tmp/|" /etc/php7/php.ini  \ 
    ...
    && sed -i "s|;error_log = php_errors.log|error_log = /data/logs/php_errors.log|" /etc/php7/php.ini  \ 
    ...
    && sed -i "s|;pid = run/php-fpm7.pid|pid = /data/conf/php-fpm7.pid|" /etc/php7/php-fpm.conf \
    && cp /etc/php7/php-fpm.conf /data/conf/ && cp /etc/php7/php-fpm.d/www.conf /data/conf/php-fpm.d/ \
    && cp /etc/php7/php.ini /data/conf/
...
CMD ["/usr/bin/php-fpm" , "-c" , "/data/conf/php.ini" , "-y" , "/data/conf/php-fpm.conf"]
